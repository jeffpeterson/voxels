    ...
    for ( var y = 0; y < h; y++) {
      ...
      for ( var d = 0; d < 3; d++) {       // look for intersections with 3 different planes / sides of cubes (x, y, z)
        ...
        if (d == 1)
          dimLength = _yd;
        if (d == 2)
          dimLength = _zd;            // yep... so _xd, _yd, and _zd represent lengths of something? arrays? rays?

        var ll = 1 / (dimLength < 0 ? -dimLength : dimLength);  // absolute inverse of dimLength
        var xd = (_xd) * ll;               // when in 'x dimension mode' this will be == _xd ( no change )
